...
type Props = {
	item: scheduledNotificationType[]
	handleSort: (key: keyof scheduledNotificationType) => void;
	sortConfig: SortConfig;
	filteredScheduled: any;
	startIdx: any;
	endIdx: any;
						...
						<table className="min-w-[950px] sm:min-w-[130px] w-full text-left border-separate border-spacing-0">
							<thead>
								<tr className="h-[42px] sm:h-[52px] bg-fgc dark:bg-fgcDark rounded-xl">
									<th className="w-[152px] sm:w-[260px] px-3 sm:px-5 py-3.5 cursor-pointer" onClick={() => handleSort("scheduleID")}>
										<div className="flex items-center gap-2">
											<span className="text-text dark:text-textDark text-xs sm:text-base font-medium whitespace-nowrap">schedule ID</span>
											<Icon
												icon={sortConfig?.key === "scheduleID" ? "up-sort" : "sort"}
									...
									<th className="w-[180px] sm:w-[220px] px-3 sm:px-5 py-3.5">
										<span className="text-text dark:text-textDark text-xs sm:text-base font-medium whitespace-nowrap">Message Title</span>
									</th>
																...
																<div className="flex flex-col items-start px-3 py-2 sm:px-2.5 sm:py-2.5 gap-1">
																	<Menu.Item>
																		<div
																			...
																			onClick={() => {
																				const globalIndex = scheduledNotification.findIndex((u: any) => u.id === item.id);
																				setEditIndex(globalIndex);
																				setIsAddEditScheduledPopupOpen(true);
																			}}
																		>
																			Edit
																		</div>
																	...
																	<Menu.Item>
																		<div
																			className="flex p-1 sm:px-3 sm:py-2.5 items-center gap-2 text-sm sm:text-base text-textSecondary dark:text-textDark cursor-pointer w-full"
																			onClick={() => {
																				setDeleteUserIndex(scheduledNotification.findIndex((u: any) => u.id === item.id));
						...
						<div className="w-full flex items-center">
							<span className="text-xs sm:text-sm text-textSecondary leading-[18px] sm:leading-[24px]">
								{filteredScheduled.length === 0
									...
									: `Showing ${startIdx + 1}–${Math.min(endIdx, filteredScheduled.length)} of ${filteredScheduled.length} Post / Article`}
							</span>
						</div>
						{filteredScheduled.length !== 0 && (
							<Pagination
								totalRecords={filteredScheduled.length}
